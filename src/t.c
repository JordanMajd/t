...
void editorRowInsertChar(erow *row, int at, int c) {
	...
	// add byte for new char and null byte
	row->chars = realloc(row->chars, row->size + 2);
	//move remaining chars to make room for new one
...
void editorUpdateRow(erow *row){
	...
	free(row->render);
	row->render = malloc(row->size + tabs*(T_TAB_STOP - 1) + 1);

	int idx = 0;
	for(j = 0; j < row->size; j++){
		if(row->chars[j] == '\t'){
			row->render[idx++] = ' ';
			//render spaces till tab stop which is every 8 columns
			while(idx % T_TAB_STOP != 0){
				row->render[idx++] = ' ';
			}
		}else{
...
void editorInsertNewline(){
	...
	if (E.cx == 0) {
		...
		//reassign pointer because editorInsertRow calls realloc
		row = &E.row[E.cy];
		row->size = E.cx;
		row->chars[row->size] = '\0';
		editorUpdateRow(row);
...
void editorMoveCursor(int key){
	...
	// keep cursor in bounds with row lenght when scrolling from long row to short
